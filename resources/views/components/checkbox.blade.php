    ...
    <div class="flex flex-row">
        @foreach($choices as $val => $choice)
            <div class="flex flex-row p-4">
                <x-input id="{{ $name.'-'.Str::snake($choice) }}" class="{{ $errors->has($name) ? 'border-red-600' : '' }}" type="checkbox" name="{{ $name }}" value="{{ $val }}" {{ $attributes->merge($value === $val || old($name) === $val ? ['checked'=>'checked'] : []) }}></x-input>
