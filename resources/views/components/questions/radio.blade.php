    ...
    <div class="flex flex-wrap w-full">
        @foreach($choices as $val => $choice)
            <div class="flex flex-row px-4 py-1">
                <x-form-input id="{{ $name.'-'.Str::snake($choice) }}" class="rounded-lg {{ $errors->has($name) ? 'border-red-600' : '' }}" type="radio" name="{{ $name }}" value="{{ $val }}" {{ $attributes->merge($value === $val ? ['checked'=>'checked'] : []) }}></x-form-input>
