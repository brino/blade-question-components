    ...
    <div class="flex flex-row">
        @foreach($choices as $val => $choice)
            <div class="flex flex-row p-4">
                <x-form-input id="{{ $name.'-'.Str::snake($choice) }}" class="{{ $errors->has($name) ? 'border-red-600' : '' }}" type="radio" name="{{ $name }}" value="{{ $val }}" {{ $attributes->merge((int) $value === $val || old($name) === $val ? ['checked'=>'checked'] : []) }}></x-form-input>
